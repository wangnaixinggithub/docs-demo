...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20240222201739782-17167197303921.png)
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20240222203423777-17167197497662.png)
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20240222203741910-17167197641443.png)
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/5-3-17086056505676-17167197903524.png)
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20240222204214446-17167198041915.png)
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20240222204353848-17086058349828-17167198179686.png)
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20240222204448365-17086058894959-17167198313997.png)
...
有时，不想为携带依赖增加其他开销。想使用编译器在寄存器中缓存这些值，以及优化重排序操作代码。



可以使用`std::kill_dependecy()`显式打破依赖链，`std::kill_dependency()`是一个简单的函数模板，会复制提供的参数给返回值。例如，一个全局的只读数组，当其他线程对数组索引进行检索时，使用的是`std::memory_order_consume`，可以使用`std::kill_dependency()`让编译器知道这里不需要重新读取该数组的内容。
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20240222204745254-17167198458798.png)
