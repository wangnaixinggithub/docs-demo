...
C风格的可变形参，编译器在解析到这样的函数时，传递的实参会依次进栈。所以对可变参数的访问，就是栈的访问。

```
typedef char* va_list; 定义访问栈的指针
va_start 该指针指向栈底
va_arg 取出栈元素存的值
va_end 指针重置为空
```
...
数组作为方法形参的时候，不仅仅可以声明为数组形式参数，还可以声明为指针。
...
void forEach(int* arr,int length) { //数组名 = 数组首元素地址值
...
二维数组作为方法形参的时候，不仅仅可以声明为二维数组形式参数，还可以声明为指针数组。
...
三维数组作为方法形参的时候，不仅仅可以声明为数组三维数组形式参数，还可以声明为指针二维数组形式。
...
如果不希望在函数中修改结构体变量的值，可以对形参加`const约束`。
...
把引用的地址复制给形式参数,这意味着，修改形式参数会影响实际参数。
