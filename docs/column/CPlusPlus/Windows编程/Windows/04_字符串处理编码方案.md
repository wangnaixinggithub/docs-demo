...
如果我设置了中文区域设置和不设置中文区域设置  `_tcscoll()` 比较的结果不同，究其理由是`_tcscoll()` 带时区特征，如为中文区域，则中文字符按照拼音进行比较（和操作系统有关，WINDOW操作系统还可以按照笔划排序）
...
![](https://blogwnx-bucket.oss-cn-beijing.aliyuncs.com/img/image-20231119182224094.png)
